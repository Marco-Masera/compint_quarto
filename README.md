...
All tests are balanced, having the agent and the adversary play an even number of games as first and as second player, since the first player seem to have a higher chance of winning.
...
* **Ties**: 5%
* **Loses**: 2.5%
* **Depth 9 reached**: 20% of times.
Comment:
It is interesting to see that 80% of times the adversary was not able to reach the required depth to activate MinMax. This shows that the agent is able to use the heuristic effectively to win as soon as possible. 
...
### Agent VS No Cache Agent
The test here is run against an agent where the RL layer is deactivated. This shows if this layer makes any significant difference.
...
### Agent VS Flat Heuristic Agent
The test against the blind agent shows if the agent is better than a simple beam search with no significant heuristics. Here the same test is done, but the adversary can use the RL layer. This should show how significant both features are.
