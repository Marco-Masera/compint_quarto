...
A collapse-state function, used to restrict the space of possible game states, has been developed and used in the training phase, but later removed from the final agent due to excessive time consumed.
...
## Overview
The basic idea is to build the final agent as a MinMax with beam-search-like pruning, using the reward functon learned so far as the heuristic to decide which child nodes to explore.
...
Comment:
Except for the RL layer, the player becomes deterministic when playing against itself, meaning it always plays the same moves, and these always goes in favour of the second player.
...
Comment: it seems the reward function does play some role, since the agent using it has an advantage. The advantage doesn't seem to be massive, since the blind agent can still manage to win around 26% of times and get to tie 31%. Interestingly, when Agent VS Agent showed a significant advantage for the second player, this time I didn't observe anything like that. The results seemed more like random. The random reward probably can be an advantage sometimes, as it allows to find good moves that the agent, by design, ignores.
