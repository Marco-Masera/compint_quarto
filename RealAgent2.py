...
class QuartoRealAgent2():
    ...
    def __init__(self, skip_rl_layer = False, random_reward_function = False):
        ...
        if (skip_rl_layer==True):
            ...
            self.perm_cache = QuartoRealAgent2.load_cache()
    ...
    def solve_states(self, states, initial_depth = None):
        ...
        if (QuartoRealAgent2.MAX_NODES[states_depth] != -1 and len(states)>QuartoRealAgent2.MAX_NODES[states_depth]):
            states = self.prune(states, QuartoRealAgent2.MAX_NODES[states_depth], states_depth)
        
        return self.solve_with_minmax(states, QuartoRealAgent2.DEPTHS[states_depth], QuartoRealAgent2.MAX_NODES[states_depth],QuartoRealAgent2.MAX_EVALS[states_depth], states_depth, 0)
        exit()
